...
export const DialogProductQuantity = ({ product, isOpen, onOpen, onClose,addToProductInCart }: Props) => {
            ...
            {isOpen &&
                <div className={'flex items-center justify-center fixed inset-0 min-h-screen w-full bg-black/40 z-10 px-4'} onClick={onClose}>
                    <div className='bg-slate-100 w-full sm:max-w-[425px] h-auto p-4 rounded-md flex flex-col relative' onClick={(e) => e.stopPropagation()}>
                        ...
                        <button type='button' title='Cancelar' onClick={addProduct} className='cursor-pointer py-2 rounded border border-red-600 bg-red-600 text-slate-100 w-20'>
                            Guardar
                        </button>
                    </div>
                    </div>
